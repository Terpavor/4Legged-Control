...
save_gif_flag = false;
    ...
    gif_filename = 'static_walk_wave.gif';
...
trajectory = static_walk_wave(R, g, [1 0 0], ground_height, traj_point_count);
...
% График
if colored_plot_flag % цветная версия
...
% отображаем корпус робота
set( hPlot(7),  'XData',robot_case(1,:), ...
                'YData',robot_case(2,:), ...
                'ZData',robot_case(3,:) );
% copyobj применяется как аналог "hold on" для plot3

axis equal;
% вручную задаем границы графика
axis_limits = [ -sum(L(2:end))-0.6	sum(L(2:end))+0.2 ...
                -sum(L)+0.5         sum(L)-0.5 ...
                ground_height           0.5];
axis(axis_limits);
...
while ishandle(hFig)
    % выбираем следующие точки траектории для каждой ноги
    for i=1:leg_count;
        g{i} = trajectory{i}(traj_i,:)';
    end
    
    % Вычисляем новые углы (инверсная кинематика)
        for i=1:leg_count;
    ...
    % Рисуем опорный многоугольник
        ...
        % тяжести находится в точке [0, 0](z сейчас не важен).
        ...
        % обновляем опорный многоугольник на графике
    ...
    % Принудительно отрисовываем график без pause
        drawnow;
    % Выводим в консоль фазу ходьбы, координаты, углы
        %clc;
       % disp(phase_i);
    ...
    % инкрементируем номер точки траектории
    traj_i = traj_i+1;
    % если траектория пройдена(закончена)
    if traj_i >= length(trajectory{1})
        
        traj_i = 1;
        e = g;
        trajectory = static_walk_wave(R, e, [0.6 0 0], ground_height, traj_point_count);
        phase_i = phase_i+1;
