function trajectory = static_walk(R, e, step_vector, ground_height, traj_point_count)
    persistent phase
    persistent leg_i
...
%     %
    ...
    switch phase
        case 0 % фаза сдвига    
            ...
            % для incenter нужен такой формат, даже если треугольник один
            TR = delaunayTriangulation( ...
                supporting_polygon(1,:)', supporting_polygon(2,:)');
            % находим инцентр треугольника
            IC = [incenter(TR,1) 0]';

            % и сдвигаем туда проекцию центра тяжести
%             for i=1:length(R)
%                 e{i} = R{i}(:,end);
%                 g{i} = e{i} + IC;
%             end
            ...
            for i = 1:4
                %g = e{i} + IC
                g = [-IC(1:2) + e{i}(1:2); ground_height];
